...
void SysDashboard()
{
  ...
  while(1)
  {
    ...
    pgs = EE_DoMoAct.DashBoard[rem[v]].PGS;
    num = EE_DoMoAct.DashBoard[rem[v]].num;
...
/**
 * show the information about the ports, groups or sensors that have been assigned to the 
 * dashboard
 * 
 * return :
...
int ShowDashboard()
{
  ...
  for (j = 0; j < EE_NUMDASHBOARD; j++) {
    ...
    if (pgs == PORTS) {
      ...
      Serial.print(num);
      Serial.print("\t");
      Serial.print(EE_DoMoAct.ports[num].Name);
    ...
    if (pgs == GROUP) {
      ...
      Serial.print(num);
      Serial.print("\t");
      Serial.print(EE_DoMoAct.groups[j].Name);
    ...
    if (EE_DoMoAct.DashBoard[j].PGS == SENSOR) {
      
      Serial.print("\tSensor\t");
      Serial.print(num);
      Serial.print("\t");
      Serial.print(EE_DoMoAct.sensors[num].Name);
...
void ConfigDashBoard() {
  ...
  while(1) { 
    ...
    if (v == 1) {     // remove
      ...
      EE_SetDashboard(EE_DoMoAct.DashBoard[rem[v]].PGS, EE_DoMoAct.DashBoard[rem[v]].num, false);
    ...
    if (v == 2) {
      ...
      if (v == 1 ) {    // port
        ...
        for (j = 0; j < EE_NUMPORTS;j++) {
          Serial.print(j);
          Serial.print("\tPort\t");
          Serial.print(EE_DoMoAct.ports[j].Name);
        ...
        for (j = 0; j < EE_NUMSENSORS;j++) {
          Serial.print(j);
          Serial.print("\tSensor\t");
          Serial.print(EE_DoMoAct.sensors[j].Name);
  ...
  while(1){
    ...
    switch(v) {
      ...
      case 5:
        ...
        break;
      case 6:
        EE_SetDisplaySwitch(true);
        break;
      case 7:
        EE_SetDisplaySwitch(false);
        break;
      default:
        break;
    }
    ...
    switch(v) {
      case 1:
        if (AllowAll && p == STRENTER) {
          for (j =0; j < EE_NUMPORTS ; j++)  DispPortInfo(j,false);
      ...
      case 3:
        s = !EE_DoMoAct.ports[p].On_sunset;
        EE_PT_Onsunset(s, p);
      ...
      case 6:
        s = !EE_DoMoAct.ports[p].Off_sunrise;
        EE_PT_Offsunrise(s, p);
      ...
      case 9:
        s = !EE_DoMoAct.ports[p].ActiveHigh;
        EE_PT_ActiveLevel(s, p);
      ...
      case 17: //Clear time setting
        ...
        if (!CheckForGroupOrSensor(p)) EE_PT_Enable(false, p);
        break;
    ...
    switch(v) {
      case 1:
        if (AllowAll && g == STRENTER) {
          for (j =0; j < EE_NUMGROUPS ; j++)  DispGroupInfo(j,false);
        } else  DispGroupInfo(g,false);
        AllowAll = false;
        break;
        
      case 2: //Enable OnSunset
        s = !EE_DoMoAct.groups[g].On_sunset;
        EE_GR_Onsunset(s, g);
        Serial.print("Sunset is set ");
        if (s) Serial.println("ON");
        else {
          ...
          if (EE_DoMoAct.groups[g].Enable){
            if (EE_DoMoAct.groups[g].On_minute == EE_NOTASSIGNED || EE_DoMoAct.groups[g].On_hour == EE_NOTASSIGNED){
      ...
      case 5: //Switch OffSunrise
        s = !EE_DoMoAct.groups[g].Off_sunrise;
        EE_GR_Offsunrise(s, g);
        Serial.print("Sunrise is set ");
        if (s) Serial.println("ON");
        else {
          Serial.println("OFF");
          ...
          if (EE_DoMoAct.groups[g].Enable){
            if (EE_DoMoAct.groups[g].Off_minute != EE_NOTASSIGNED || EE_DoMoAct.groups[g].Off_hour != EE_NOTASSIGNED){
...
void SysSetupSensor()
{
  while(1) {
    ...
    switch(v) {
      case 1:
        if (AllowAll && g == STRENTER) {
          for (j =0; j < EE_NUMSENSORS ; j++)  DispSensorInfo(j,false);
...
void SysSetupInit()
{
  ...
  Serial.println("About the EEPROM backup location");
  Serial.println("1\tYES. Overwrite the EEPROM backup location.");
  Serial.println("2\tNO. Do NOT overwrite the EEPROM backup location.");
  ...
  if (v == 1) EE_InitEeprom(true, true);
  else EE_InitEeprom(true, false);
} 
...
void ManualSwitch()
{
  bool disp = EE_DoMoAct.DisplaySwitch;
  EE_DoMoAct.DisplaySwitch = true;       // enable showing switch
  ...
  while(1) {
    ...
    v = GetInput(4);
    if (v < 0 || v == 4) {
      EE_DoMoAct.DisplaySwitch = disp; // restore original setting
      return;
    }
    s = false;
    switch(v) {
      ...
      case 3:
        ...
        if (EE_DoMoAct.sensors[g].Enable) {
...
void SysMaintenance()
{
  while(1){
    ...
    Serial.println("9\tMake a backup in EEPROM");
    Serial.println("10\tRestore a backup from EEPROM");
    ...
    Serial.println("18\tManually toggle Daylight Saving");
    Serial.println("19\tDone");
    v = GetInput(19);
    if (v < 0 || v == 19 ) return;
    ...
    switch(v) {
      ...
      case 18:  // Manually set DayLight Saving (happens automatic already, but just in case)
        if (EE_DoMoAct.DST == 1){
        ...
        TriggerUpdateTime = true;  // update time and sunset in loop 
        break;       
...
int8_t GetPortGroupSensor(byte v)
{
  ...
  if (v == PORTS) {
    ...
    Serial.print(EE_NUMPORTS -1); // starts with port 0
    h = GetInput(EE_NUMPORTS -1);
  }
  else if (v == GROUP) {
    Serial.print("a valid Group number. Max. ");
    Serial.print(EE_NUMGROUPS -1); // starts with group 0
    h = GetInput(EE_NUMGROUPS -1);
  }
  else {
    Serial.print("a valid Sensor number. Max. ");
    Serial.print(EE_NUMSENSORS -1); // starts with sensor 0
    h = GetInput(EE_NUMSENSORS -1);
  }  
...
void DisplayWorkDays(int w, bool n)
{
    ...
    if (n) Serial.print(d++);
    Serial.print("\tSunday\t\t");
    if (((w >> EE_SUNDAY) & 0x1) == 0x1) Serial.println("Enabled");
    else Serial.println("Disabled");

    if (n) Serial.print(d++);
    Serial.print("\tMonday\t\t");
    if (((w >> EE_MONDAY) & 0x1) == 0x1) Serial.println("Enabled");
    else Serial.println("Disabled");

    if (n) Serial.print(d++);
    Serial.print("\tTuesday\t\t");
    if (((w >> EE_TUESDAY) & 0x1) == 0x1)Serial.println("Enabled");
    else Serial.println("Disabled");

    if (n) Serial.print(d++);
    Serial.print("\tWednesday\t");
    if (((w >> EE_WEDNESDAY) & 0x1) == 0x1) Serial.println("Enabled");
    else Serial.println("Disabled");

    if (n) Serial.print(d++);
    Serial.print("\tThursday\t");
    if (((w >> EE_THURSDAY) & 0x1) == 0x1) Serial.println("Enabled");
    else Serial.println("Disabled");

    if (n) Serial.print(d++);
    Serial.print("\tFriday\t\t");
    if (((w >> EE_FRIDAY) & 0x1) == 0x1) Serial.println("Enabled");
    else Serial.println("Disabled");

    if (n) Serial.print(d++);
    Serial.print("\tSaturday\t");
    if (((w >> EE_SATURDAY) & 0x1) == 0x1) Serial.println("Enabled");
    else Serial.println("Disabled");
}
...
void HandleWorkDays(byte dev, int8_t n)
{
  while(1) {
    ...
    // get current values
    if (dev == PORTS) v = EE_DoMoAct.ports[n].Day;
    else if (dev == GROUP) v = EE_DoMoAct.groups[n].Day;
    else v = EE_DoMoAct.sensors[n].Day;
    ...
    if( d != EE_NOTASSIGNED){
      ...
      if (dev == PORTS) EE_PT_SetDay(s, d, n);
      else if (dev == GROUP) EE_GR_SetDay(s, d, n);
      else EE_SN_SetDay(s, d, n);
    }
  }
}
...
bool CheckForGroupOrSensor(int p)
{
  ...
  for(j = 0; j < EE_NUMSENSORS; j++){
    ...
    for(byte i = 0; i < EE_NUMPORTS; i++){
      if (EE_DoMoAct.sensors[j].Ports[i] == p)
      return true;
    }
  }
...
bool PortValidEnable(int p)
{
  ...
  // check GPIO
  if (EE_DoMoAct.ports[p].Gpio == EE_NOTASSIGNED) {
    valid = false;
  }
  
  // enable or disable port
  EE_PT_Enable(valid, p);
...
void UpdateName(int h, byte dev)
{
  ...
  if (dev == PORTS) Serial.print(EE_DoMoAct.ports[h].Name);
  else if (dev == GROUP) Serial.print(EE_DoMoAct.groups[h].Name);
  else Serial.print(EE_DoMoAct.sensors[h].Name);
  Serial.println(". Provide a new name. only <enter> so cancel");
  n = Serial.readStringUntil(0x0d);
  
  if (n.length() > EE_NAMELEN )
    Serial.println("Name is too long");
  else if (n.length() > 0) {
    if (dev == PORTS) EE_PT_Name(n, h);
    else if (dev == GROUP) EE_GR_Name(n, h);
    else EE_SN_Name(n, h);
    return;
  }
...
void SysMainHealthCheck()
{
  ...
  for (j = 0 ; j < EE_NUMGROUPS ; j++) {
      if (EE_DoMoAct.ports[p].Group[j] != EE_NOTASSIGNED) group[EE_DoMoAct.ports[p].Group[j]]++;
  }
  ...
  for (j = 0 ; j < EE_NUMGROUPS ; j++) {
    ...
    if (EE_DoMoAct.groups[j].count != group[j]) {
      Serial.print("WARNING: Mismatch on count for group ");
      Serial.print(j);
      Serial.print(" detected ");
      Serial.print(group[j]);
      Serial.print(" current count is ");
      Serial.print(EE_DoMoAct.groups[j].count);
      Serial.println(" (Corrected!)");  
      
      EE_DoMoAct.groups[j].count = group[j];
    }
  ...
  for (p = 0 ; p < EE_NUMSENSORS ; p++){
    ...
    for(j = 0; j < EE_NUMPORTS; j++){
      if (EE_DoMoAct.sensors[p].Ports[j] != EE_NOTASSIGNED) {
        Serial.print("INFO: Assigned Port ");
        Serial.println(EE_DoMoAct.sensors[p].Ports[j]);
        c++;
      }
    }
  ...
  // check for correct backup in EEPROM
  if (! EE_ReadBackUpEeprom(false, true))
  {
    Serial.println("EEPROM backup does not look to be correct");
    EE_WriteBackUpToEEprom(true);   // force an update
  }

  // check ability to connect to NTP server
  if (getNtpTime() == 0) {
    Serial.println("The connection with NTPServer does not look to be working.");
  }
...
/**
 * check that a conditions are valid to enable
 * 
 * return:
 * true : no concern
 * false : some concerns
...
bool GPShealth(int p, int gps)
{
  ...
  if (gps == PORTS) {
    ...
    if (EE_DoMoAct.ports[p].Gpio == EE_NOTASSIGNED) {
      ...
      Serial.print(EE_DoMoAct.ports[p].Gpio);
    ...
    if (EE_DoMoAct.ports[p].Day != 0x7f) {
    ...
    if (EE_DoMoAct.ports[p].On_minute == EE_NOTASSIGNED || EE_DoMoAct.ports[p].On_hour == EE_NOTASSIGNED) {
    ...
    // no sunset was set
    if (! EE_DoMoAct.ports[p].On_sunset) {
    ...
    if (EE_DoMoAct.ports[p].Off_minute == EE_NOTASSIGNED || EE_DoMoAct.ports[p].Off_hour == EE_NOTASSIGNED){
    ...
    // no sunrise was set
    if (! EE_DoMoAct.ports[p].Off_sunrise){ 
  ...
  if (gps == GROUP){
    ...
    if (EE_DoMoAct.groups[p].Day != 0x7f) {
      Serial.println("INFO: not all days are enabled.");
    }
    else
      Serial.println("INFO: All days are enabled.");
    ...
    // no fixed On time was set
    if (EE_DoMoAct.groups[p].On_minute == EE_NOTASSIGNED || EE_DoMoAct.groups[p].On_hour == EE_NOTASSIGNED) {
      Serial.println("INFO: NO OnMinute or OnHour set.");
    }
    else {
      Serial.println("INFO: OnMinute and OnHour are set.");
    ...
    // no sunset was set
    if (! EE_DoMoAct.groups[p].On_sunset) {
      Serial.println("INFO: NO On_Sunset");
    } 
    else {
      Serial.println("INFO:set for On_Sunset");
    ...
    // no fixed OFF time was set
    if (EE_DoMoAct.groups[p].Off_minute == EE_NOTASSIGNED || EE_DoMoAct.groups[p].Off_hour == EE_NOTASSIGNED){
      Serial.println("INFO: NO OffMinute or OffHour set");
    }
    else {
      Serial.println("INFO: OFFMinute and OFFHour are set.");
    ...
    // no sunrise was set
    if (! EE_DoMoAct.groups[p].Off_sunrise){ 
      Serial.println("INFO: NO On_Sunrise");
    }
    else {
      Serial.println("INFO:set for On_Sunrise");
