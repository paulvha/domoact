/*
  ...
  Created December 2023 / version 1.1
  by paulvha

  todo : web SSL security
...
MyObject EE_DoMoAct = {0};      // Parameter structure stored in EEPROM
...
void setup() {
  ...
  EE_ReadEeprom(false);      // restore parameters (do not display)
...
void loop()
{
  ...
  if (TriggerUpdateTime) {
    ...
    EE_WriteBackUpToEEprom(false);    // Parameters to backup location if changed
