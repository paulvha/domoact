...
// define as 1 if you want to create a new certificate
// define as 0 to test your certificate stored in certificates.h
#define CREATE_CERT 1
...
void setup() {
  ...
  CreateCertificate();
#else
  // test certificate from certificates.h
...
void handleRoot(HTTPRequest * req, HTTPResponse * res) {
  ...
  res->println("<body>");
  res->println("<h1>Hello World! Your certificate is working </h1>");
...
void  CreateCertificate()
{
  ...
  /* paulvha:
   *
   *  Given that the ESP32 only may have 1 KB to store EEPROM values, I decided to create output that can be included in a seperate
   *  tab in the final server solution for test.
   */

  int i, j;
  uint16_t l;

  Serial.print("\n\n/* Certificate generated for test on ");
  ...
  l = cert->getPKLength();
  Serial.print("uint16_t pkLength = ");
  Serial.print(l);
  Serial.println(";");
  unsigned char * pkdata = cert->getPKData();

  Serial.print("unsigned char pkData[] = {\n");

  for (i = 0, j = 0 ; i < l ; i++,j++) {
    Serial.print("0x");
    Serial.print(pkdata[i],HEX);
  ...
  // creat certificate
  l = cert->getCertLength();
  Serial.print("uint16_t certLength = ");
  Serial.print(l);
  Serial.println(";");
  unsigned char * pcdata = cert->getCertData();

  Serial.print("unsigned char certData[] = {\n");

  for (i = 0, j = 0 ; i < l ; i++,j++) {
    Serial.print("0x");
    Serial.print(pcdata[i],HEX);
    if (i != l -1) Serial.print(", ");
  ...
  Serial.println("\nDONE, Now copy your PK and CERT to tab certificates.h.");
  Serial.println("Define CREATE_CERT as 0, recompile and test. freeze");
  while (1);
}
#endif
