...
bool UpdateSunTimes()
{
  ...
  {
    sunrise = ((int) (today[tl_hour] + EE_DoMoAct.DST)* 60) + (int) today[tl_minute]; // adjust Daylight Saving Time
  }
   
  if (tardis.SunSet(today)) // if the sun will set today (it might not, in the [ant]arctic)
  {
    sunset = ((int) (today[tl_hour] + EE_DoMoAct.DST)* 60) + (int) today[tl_minute];  // adjust Daylight Saving Time
  }
...
////////////// turn ON Daylightsaving
  ...
  if (calcDayWeek() == 0  && calcMonth() == 2 && calcDay() >= 25 && calcHours() > 2 && EE_DoMoAct.DST==0)
  ...
  if (calcDayWeek() == 0  && calcMonth() == 2 && calcDay() >= 7 && calcHours() > 2 && EE_DoMoAct.DST==0)
#else
#error: current DAYLIGHTSAVING setting NOT supported (check in tab DoMoAct.h)
#endif
  {
    EE_Set_DST(1);    // set daylight saving
...
////////////// turn off Daylightsaving
  ...
  if (calcDayWeek() == 0 && calcMonth() == 9 && calcDay() >= 25 && calcHours() > 3 && EE_DoMoAct.DST==1)
  ...
  if (calcDayWeek() == 0 && calcMonth() == 10  && calcHours() > 3 && EE_DoMoAct.DST==1)
#endif
  {
    EE_Set_DST(0);    // remove daylight saving
...
unsigned long getNtpTime()
{
  ...
  while (millis() - beginWait < 1500) {
    ...
    if (size >= NTP_PACKET_SIZE) {
      ...
      Udp.stop();
      return secsSince1900 - 2208988800UL + ((UTC_OFFSET + EE_DoMoAct.DST) * 3600);
    }
  }
